...
function BlockCard({ block }: { block: (typeof BLOCKS_CATALOG.blocks)[number] }) {
  ...
  return (
    <Card className="w-full p-5 rounded-card bg-carbon border-graphite flex flex-col justify-between space-y-4 shadow-none">
      {/* Top Details */}
      <div className="space-y-2">
        <CardHeader className="p-0 flex flex-row items-center justify-between space-y-0">
          <CardTitle className="font-mono text-[12px] text-paper font-normal">
            {block.name}
          </CardTitle>
          <Badge
            variant="outline"
            className="px-1.5 py-0.5 rounded-badge bg-obsidian border-graphite font-mono text-[10px] text-fog font-normal"
          >
            {block.tag}
          </Badge>
        </CardHeader>
        ...
        <CardContent className="p-0">
          <p className="text-[13px] text-fog leading-relaxed">{block.description}</p>
        </CardContent>
      </div>

      {/* Bottom Command Strip & Copy Button */}
      <CardFooter className="p-0 py-3 border-t border-graphite flex justify-between items-center text-[11px] font-mono text-ash">
        <code className="text-[11px] font-mono text-ash bg-transparent p-0">{block.command}</code>
        ...
        <div className="relative">
          <Button
            ...
            size="icon"
            onClick={handleCopy}
            className="h-6 w-6 p-0 hover:bg-obsidian hover:text-mist text-ash transition-colors"
