...
```bash
npx blockend-cli add <block>
```

For example:

```bash
npx blockend-cli add auth
```
...
Generate a env-config block:

```bash
npx blockend add env-config
```

Generate an upload block:

```bash
npx blockend-cli add upload
```
...
```text
blockend.json not found.
Run 'npx blockend-cli init' first.
```
