...
<SourceCode file="blocks/error-handler/express/app-error.ts" />

The CLI generates this file without the `express/` subdirectory — the source path shown reflects the monorepo source.

### blocks/error-handler/http-status.ts

Named HTTP status code constants.

<SourceCode file="blocks/error-handler/express/http-status.ts" />
...
Register the handler last, after all your routes and other middleware.
...
**Throws** — Always use `throw new AppError(...)` or `throwError()`.

#### ERRORS

Record of common error catalog entries.
...
### Production Usage with Logger

```ts
import { globalErrorHandler, AppError } from "@/blocks/error-handler";
import { logger } from "@/blocks/logger/core";

// Replace internal console.error with structured logger
...
- **Request Validator** — Pair with `propagateErrors: true` to centralize validation error handling.
